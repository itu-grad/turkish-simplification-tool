...
export default function TextAnalysisOutputComponent() {
    ...
    const handlePrevious = () => {
        setIsFading(true);
        setTimeout(() => {
            setCurrentIndex((prevIndex) => (prevIndex === 0 ? alternatives.length - 1 : prevIndex - 1));
            setIsFading(false);
        }, 300);
    };
        ...
        <div className="p-8 min-w-[1200px] bg-[#f5f5f5] rounded-xl shadow-lg flex flex-col space-y-6 mt-10 mb-10">
            <div className="flex flex-row gap-3">
                ...
                <div className={`w-full transition-opacity duration-300 ${isFading ? "opacity-0" : "opacity-100"}`}>
                    <div className="text-2xl font-medium text-[#1e1e1e] mb-5">Analiz Sonucu</div>
                    <div className="p-6 bg-white rounded-md shadow-sm">
                        <p className="text-[#1e1e1e] text-justify">{alternatives[currentIndex].text}</p>
                    </div>
                        ...
                        <div className="flex flex-col p-6 bg-white rounded-md shadow-sm">
                            ...
                            <div className="overflow-y-scroll mt-4 max-h-32">
                                {alternatives[currentIndex].words.map((word, index) => (
                                    <div key={index} className="text-[#1e1e1e]">{word}</div>
                                ))}
                            </div>
                        </div>
            ...
            <div className="flex flex-row">
                <button
                    className="mt-6 p-3 bg-gray-500 text-white rounded-md transition-all duration-300 hover:scale-105 hover:bg-gray-600 active:scale-95 cursor-pointer ml-auto"
